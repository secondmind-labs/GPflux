...
def construct_basic_kernel(kernels, output_dims=None, share_hyperparams=False):
    if isinstance(kernels, list):
        mo_kern = SeparateIndependent(kernels)
    elif not share_hyperparams:
        copies = [deepcopy_components(kernels) for _ in range(output_dims)]
        mo_kern = SeparateIndependent(copies)
    else:
        mo_kern = SharedIndependent(kernels, output_dims)
    return mo_kern


def construct_basic_inducing_variables(
    num_inducing, input_dims, output_dims=None, share_variables=False
):
    if isinstance(num_inducing, list):
        inducing_variables = []
        for num_ind_var in num_inducing:
            empty_array = np.empty((num_ind_var, input_dims), dtype=dfloat())
        ...
        for _ in range(output_dims):
            empty_array = np.empty((num_inducing, input_dims), dtype=dfloat())
            inducing_variables.append(InducingPoints(empty_array))
        return SeparateIndependentInducingVariables(inducing_variables)
