...
class DistConfig:
    ...
    type_likelihood: str
    """
    Can either be 'Gaussian' or 'StudentT'
    """
...
def _construct_hybrid_kernel(input_dim: int) -> Hybrid:
    ...
    return Hybrid("squared_exponential", lengthscales=lengthscales, variance=variance)


def build_dist_deep_gp(X: np.ndarray, num_layers: int, config: DistConfig) -> DistDeepGP:
    r"""
    ...
    :param config: The configuration for (hyper)parameters. See :class:`DistConfig` for details.
    """
    ...
    if config.type_likelihood == "Gaussian":
        likelihood = Gaussian(config.likelihood_noise_variance)
    elif config.type_likelihood == "StudentT":
        likelihood = gpflow.likelihoods.StudentT(scale=1e-2)
    else:
