...
class RandomFourierFeatures(tf.keras.layers.Layer):
    ...
    * 𝜏 ∼ 𝒰(0, 2π)
    ...
    def __init__(self, kernel: gpflow.kernels.Kernel, output_dim: int, **kwargs):
        """
    ...
    def build(self, input_shape):
        ...
        if not isinstance(self.kernel, gpflow.kernels.SquaredExponential):
            tf.assert_equal(
                input_dim, 1, "Matern kernels only support 1 dimensional inputs"
            )
    ...
    def _sample_bias(self, shape, **kwargs):
        return tf.random.uniform(shape=shape, maxval=2 * np.pi, **kwargs)

    def _sample_weights(self, shape, **kwargs):
