...
from gpflow.inducing_variables import MultioutputInducingVariables, SharedIndependentInducingVariables, SeparateIndependentInducingVariables
from gpflow.kernels import Kernel, SeparateIndependent, SharedIndependent, MultioutputKernel
...
@efficient_sample.register(MultioutputInducingVariables, (SharedMultiOutputKernelWithFeatureDecomposition, SeparateMultiOutputKernelWithFeatureDecomposition), object)
def _efficient_multi_output_sample_matheron_rule(
    inducing_variable: MultioutputInducingVariables,
    kernel: Union[SharedMultiOutputKernelWithFeatureDecomposition, SeparateMultiOutputKernelWithFeatureDecomposition],
    q_mu: tf.Tensor,
    *,
    ...
    if isinstance(kernel, SharedIndependent):
        Kmm = tf.tile(Kuu(inducing_variable, kernel, jitter=default_jitter())[None,...] ,[P, 1, 1]) # [P,M,M] 
        ...
        raise ValueError("kernel not supported. Must be either SharedIndependent or SeparateIndependent")
    ...
    if whiten:
        ...
        u_sample = tf.transpose(tf.matmul(Luu, # [P, M, M] 
            tf.transpose(u_sample)[...,None] # [P, M, 1]
        )[...,0]) # [M, P]
        tf.debugging.assert_equal(tf.shape(u_sample), [M, P]) 
    ...
    v = tf.transpose(compute_A_inv_b(Kmm, #[P, M, M]
        diff # [P, M, 1]
        )[...,0])  # [M, P]
    ...
    class WilsonSample(Sample):
        def __call__(self, X: TensorType) -> tf.Tensor:
            ...
            weight_space_prior_X = tf.transpose(tf.matmul(phi_X, # [P, N, L]
                tf.transpose(prior_weights)[..., None] # [P, L, 1]
                )[...,0])  # [N, P]

            Knm = tf.linalg.matrix_transpose(Kuf(inducing_variable, kernel, X))  # [P, N, M] or [N,M]
            ...
            function_space_update_X = tf.transpose(tf.matmul(Knm, # [P, N, M]
                tf.transpose(v)[..., None]  # [P, M, 1]
                )[...,0])  # [N, P]            
