...
def rotate_img_angles(Ximgs, angles, interpolation_method):
    ...
    return tf.cast(tf.transpose(tf.map_fn(rotate, angles, dtype=tf.float32), (1, 0, 2)),
                   settings.float_type)


def rotate_img_angles_stn(Ximgs, angles):
    """
    Uses spatial transformer networks to rotate a batch of images by different angles.
    The entire batch is rotated by all angles
    ...
    def rotate(angle):
        ...
        theta = tf.stack(
            [tf.cos(angle_rad), -tf.sin(angle_rad), 0., tf.sin(angle_rad), tf.cos(angle_rad), 0.])
        theta = tf.reshape(theta, [1, -1])
        theta = tf.cast(tf.tile(theta, [tf.shape(Ximgs)[0], 1]), tf.float32)

        return tf.cast(tf.reshape(
            tf.squeeze(stn(Ximgs, theta)),
            [tf.shape(Ximgs)[0], tf.shape(Ximgs)[1] * tf.shape(Ximgs)[2]]
        ), settings.float_type)  # [?, H*W]

    return tf.transpose(tf.map_fn(rotate, angles, dtype=settings.float_type),
                        (1, 0, 2))  # [?, P, H*W]


def _apply_stn(Ximgs, theta):
    """
    Use spatial transformer networks to apply a general affine transformation (6 parameters).
    All images are transformed
    by the SAME theta
    ...
    return tf.reshape(tf.squeeze(stn(Ximgs, theta)),
                      [tf.shape(Ximgs)[0], tf.shape(Ximgs)[1] * tf.shape(Ximgs)[2]])
...
def apply_stn_batch(Ximgs, thetas):
    """
    Use spatial transformer networks to apply a general affine transformation (6 parameters).
    Every image is transformed with every theta
    :param Ximgs: input images [None, H, W] or [None, H, W, 1]
    ...
    return tf.transpose(
        tf.map_fn(lambda x: _apply_stn(Ximgs, x), thetas, dtype=settings.float_type), (1, 0, 2))
