...
def test_smoke_build_constant_input_dim_orth_deep_gp(input_dim, num_layers):
    config = make_dataclass_from_class(Config, DemoConfig)
    X = np.random.randn(13, input_dim)
    Y = np.random.randn(13, 1)
    dgp = build_constant_input_dim_orth_deep_gp(X, num_layers, config)
...
def test_build_constant_input_dim_orth_deep_gp_raises_on_incorrect_dtype(dtype):
    config = make_dataclass_from_class(Config, DemoConfig)
    X = np.random.randn(13, 2).astype(dtype)

    with pytest.raises(ValueError):
        build_constant_input_dim_orth_deep_gp(X, 2, config)
