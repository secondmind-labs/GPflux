...
from gpflux.convolution import (ConvKernel, K_image_inducing_patches,
                                K_image_symm)
...
def test_K(session_tf, conv_kernel):
    ...
    np.testing.assert_array_equal(output.shape, [conv_kernel.patch_handler.config.num_patches, Data.N, Data.N])


def test_K_diag(session_tf, conv_kernel):
    ones_patch = np.ones((1, Data.h * Data.w))
    desired_value = session_tf.run(K_image_symm(conv_kernel.basekern, conv_kernel.patch_handler, ones_patch))
    desired_value = desired_value.flatten()[0]
    ...
    np.testing.assert_array_equal(output.shape, [Data.N, conv_kernel.patch_handler.config.num_patches])
