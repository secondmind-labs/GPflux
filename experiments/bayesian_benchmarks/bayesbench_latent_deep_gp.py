...
class BayesBench_ConditionalLatentDeepGP():
    ...
    def fit(self, X, Y, Xt=None, Yt=None, name=""):
        ...
        kern2 = gpflow.kernels.RBF(Config.D_in, lengthscales=Config.D_in ** 0.5, variance=0.1)
        ...
        # build model
        self.model = gpflux.ConditionalLatentDeepGP(X, Y, encoder,
                                                    layers=[layer1, layer2],
                                                    batch_size=Config.MB)
    ...
    def _create_monitor_tasks(self, file_writer, Config):
        ...
        test_loglik_func = lambda *args, **kwargs: self.avg_log_pdf(self.Xt, self.Yt)
        ...
        kl_z_func = lambda *args, **kwargs: self.model.compute_KL_Z()
        kl_z_task = mon.ScalarFuncToTensorBoardTask(file_writer, kl_z_func, "KL_Z")\
              .with_name('kl_z')\
              .with_condition(mon.PeriodicIterationCondition(hz))\
              .with_exit_condition(True)
        ...
        return [print_task, model_tboard_task, lml_tboard_task,
                  test_loglik_task, kl_u_task, kl_z_task, data_fit_task]
