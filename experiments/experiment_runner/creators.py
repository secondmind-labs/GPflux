...
class RegularisedWeightedSumConvLayer(WeightedSumConvLayer):

    @params_as_tensors
    def KL(self):
        return gauss_kl(self.q_mu, self.q_sqrt) \
               + tf.pow(self.kern.basekern.variance, 2)
...
def create_regularised_convgp_layer(input_size, num_ind_points, patch_shape, num_classes,
                                    with_indexing,
                                    with_weights,
                                    patches):
    layer0 = RegularisedWeightedSumConvLayer(
        input_size,
        num_ind_points,
        patch_shape,
        num_latents=num_classes,
...
class RegularisedShallowConvGP(ModelCreator):
    @staticmethod
    @gpflow.defer_build()
    def create(data_source, config):
        dataset = data_source.get_data()
        ...
        layer0 = create_regularised_convgp_layer(
            [h, h],
            config.num_inducing_points,
            config.patch_shape,
            num_classes,
