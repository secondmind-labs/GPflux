...
#
# where $\sigma^2$ refers to the kernel variance and $l$ to the kernel lengthscale. $\theta_d$ and $\tau_d$ are randomly drawn hyperparameters that determine each feature function $\phi_d$. The hyperparameter $\theta_d$ is randomly drawn from the kernel's spectral density. The spectral density of a stationary kernel is obtained by interpreting the kernel as a function of one argument only (i.e. the distance between $X$ and $X^\prime$) and performing a Fourier transform on that function, resulting in an unnormalized probability density (from which samples can be obtained). The hyperparameter $\tau_d$ is obtained by sampling from a uniform distribution $\tau_d \sim \mathcal{U}(0,2\pi)$. Note that both $\theta_d$ and $\tau_d$ are fixed and not optimized over. An interesting direction of future research is how to automatically identify those (but this is outside the scope of this notebook). If we drew infinitely many samples, i.e. $D \rightarrow \infty$, we would recover the true kernel perfectly.
#
...
kernel = kernel_class(lengthscales=lengthscale)  # kernel object to draw training data set from
